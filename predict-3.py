...
with open(r'./aes.min.js', 'r', encoding='utf-8') as f:
    aes_min_js = f.read()

with open('./encrty.js', 'r', encoding='utf-8') as fs:
    encrpty_js = fs.read()
    ...
    def inner(*args, **kwargs):
        ...
        if not ret:
            while number < max_retry:
                number += 1
                logger.info("尝试第:{}次".format(number))
...
@retry
def request_ths(uname, passwd):
    uname = js_load.call('thsencrypt.encode', uname)
    passwd = js_load.call('thsencrypt.encode', passwd)
    v = ct.call("v")
    logger.info(v)
    captcha_headers = captcha_header(v)

    source = [str(i) for i in range(0, 10)]
    source += [chr(i) for i in range(97, 97 + 26)]
    alphabet = ''.join(source)
    screenImg = "%s.png" % "ths_captcha"
    a = requests.get("http://upass.iwencai.com/captcha/buildcaptcha?type=flat&16768&0.4653998146883518",
    ...
    pred = predict()
    logger.info("预测的验证码为%s" % pred)

    url = "http://upass.iwencai.com/login/dologinreturnjson2"

    payload = ths_payload(passwd, pred, uname)
    headers = ths_header(v)
...
def ths_payload(passwd, pred, uname):
    payload = {
        "uname": "%s" % uname,
        "passwd": "%s" % passwd,
        "captcha": "%s" % pred,
